const API = PRIMEVEST_API_BASE; // now cross-origin — this panel deploys separately from the backend
...
async function openUser(id) {
  ...
  if (user.identityStatus !== "unverified") {
    ...
    presentKinds = documents.map((d) => d.kind);
    const thumb = (kind, label) => {
      const has = presentKinds.includes(kind);
      return has
        ? `<a href="#" onclick="return false" style="display:block">
             <img id="doc-thumb-${kind}" alt="${label}"
                  style="width:100%;border-radius:10px;border:1px solid var(--border);object-fit:cover;height:110px;background:var(--elevated)" />
  ...
  // Fetch each present document as an authenticated blob and wire it into
  // its <img> placeholder — more reliable cross-origin than a bare <img
  // src> relying on the browser to attach cookies to a subresource load.
  presentKinds.forEach(async (kind) => {
    const img = document.getElementById(`doc-thumb-${kind}`);
    if (!img) return;
    try {
      const res = await fetch(`${API}/api/admin/users/${id}/documents/${kind}`, { credentials: "include" });
      if (!res.ok) return;
      ...
      // leave the placeholder blank — not worth failing the whole modal over
    }
  });
}
